...
"""Generator for creating a structured box grid of particles."""
...
def generateBoxParticleGrid(
    ...
    """Generate a structured box grid of particles.
    ...
    for x in range(nX):
        for y in range(nY):
            for z in range(nZ):
                ...
                pCoordinates = np.copy(grid[:, x, y, z]).reshape((-1, 3))
    ...
    model.particleSets["{:}_left".format(name)] = ParticleSet(
        "{:}_left".format(name), [n for n in particleGrid[0, :, :].flatten()]
    )
    model.particleSets["{:}_right".format(name)] = ParticleSet(
        "{:}_right".format(name), [n for n in particleGrid[-1, :, :].flatten()]
    )
    model.particleSets["{:}_top".format(name)] = ParticleSet(
        "{:}_top".format(name), [n for n in particleGrid[:, -1, :].flatten()]
    )
    model.particleSets["{:}_bottom".format(name)] = ParticleSet(
        "{:}_bottom".format(name), [n for n in particleGrid[:, 0, :].flatten()]
    )
    model.particleSets["{:}_leftBottom".format(name)] = ParticleSet(
        "{:}_leftBottom".format(name), [n for n in particleGrid[0, 0, :]]
    )
    model.particleSets["{:}_leftTop".format(name)] = ParticleSet(
        "{:}_leftTop".format(name), [n for n in particleGrid[0, -1, :]]
    )
    model.particleSets["{:}_rightBottom".format(name)] = ParticleSet(
        "{:}_rightBottom".format(name), [n for n in particleGrid[-1, 0, :]]
    )
    model.particleSets["{:}_rightTop".format(name)] = ParticleSet(
        "{:}_rightTop".format(name), [n for n in particleGrid[-1, -1, :]]
    )
