...
class ParticlePenaltyEqualValue(MPMConstraintBase):
    ...
    def __init__(
        ...
        field: str,
        prescribedComponents: int | list[int],
        ...
        self._field = field
        if isinstance(prescribedComponents, int):
            self._prescribedComponents = [prescribedComponents]
        elif isinstance(prescribedComponents, list):
            self._prescribedComponents = prescribedComponents
        else:
            raise Exception("Invalid prescribed components format")
    ...
    def updateConnectivity(self, model):
        nodes = {
            n: i for i, n in enumerate(sorted(set(kf.node for p in self._constrainedParticles for kf in p.kernelFunctions), key=lambda n: n.label))
        }
