...
def fwhm(xy, om, bw, nAng=72):
    """narrowest full width at half maximum of the damage profile, over all directions"""
    best = None
    for a in range(nAng):
        t = np.pi * a / nAng
        xi = xy @ np.array([np.cos(t), np.sin(t)])
        bins = np.arange(xi.min(), xi.max() + bw, bw)
        idx = np.digitize(xi, bins)
        prof = np.array([om[idx == k].mean() if (idx == k).any() else np.nan
                         for k in range(1, len(bins))])
        prof = prof[~np.isnan(prof)]
        if len(prof) < 6:
            continue
        hi, lo = prof.max(), prof.min()
        w = bw * int(np.sum(prof > lo + 0.5 * (hi - lo)))
...
def main():
    ...
    for a, (tag, l, ef, h) in zip(ax, CASES):
        ...
        a.set_title(f"$l_d$ = {l:.2f} mm,  $h_p$ = {h:.2f} mm  ({len(xy0)} particles)\n"
                    ...
                    f"band FWHM = {w:.2f} mm  =  {w/l:.2f} $l_d$", fontsize=9.5)
        rows.append((l, ef, h, len(xy0), Gf, w, om[k].max(), s[k]))
        last = pc
    ...
    fig.suptitle(r"Unconfined plane strain, $\beta = 45^\circ$, compared at matched damage "
                 ...
                 "same fracture energy, half the internal length: the band halves "
                 "(magenta: stored bedding direction)", fontsize=10.5, y=1.04)
    out = os.path.join(HERE, "fig_bandwidth_gf.pdf")
    fig.savefig(out, bbox_inches="tight")
    fig.savefig(out.replace(".pdf", ".png"), dpi=145, bbox_inches="tight")
    print(f"  wrote {out}\n")
    print("     l   eps_f*     h_p   part.    Gf      FWHM   FWHM/l   omega   eps%")
    for l, ef, h, n, Gf, w, o, e in rows:
        print(f"  {l:5.2f}  {ef:.2e}  {h:5.2f}  {n:5d}  {Gf:5.1f}  {w:6.2f}  {w/l:6.2f}  "
              f"{o:6.2f}  {e:5.2f}")
