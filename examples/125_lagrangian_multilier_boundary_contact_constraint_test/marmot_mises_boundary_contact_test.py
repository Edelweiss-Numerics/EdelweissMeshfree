...
from edelweissmeshfree.constraints.particlelagrangiancartesianboundary import (
    ParticleLagrangianContactCartesianBoundaryConstraintFactory,
)
...
def run_sim(no_limit=False):
    ...
    theMaterial = {
        ...
        "properties": np.array([K, G, 20e1, 1e-1, 1e-0, 0, 1, 1e-9 * massDensityScalingFactor]),
    }
    ...
    boundaryRight = ParticleLagrangianContactCartesianBoundaryConstraintFactory(
        "right", 7.0, 0, theModel.particleSets["slab_all"], "displacement", theModel, location="face", faceID=2
    )
