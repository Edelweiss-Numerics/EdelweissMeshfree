...
E = 400.0  # Soft rubber
NU = 0.45
RHO = 1e-3  # Mass scaling
PENALTY = 1e4
MU = 0.3  # Friction coefficient

DISP_Y = -0.5  # Slight indentation
DISP_X = 4.0  # Slide right
...
if is_full:
    ...
    # Small number of timesteps to finish quickly (< 2 seconds)
    STEP_TIME = 0.5
    DT = 0.01  # 50 steps total
    save_interval = 2
...
def run_friction_test():
    ...
    def disp_amp_y(t):
        return smooth_step(t, 0.0, 0.5)

    def disp_amp_x(t):
        return smooth_step(t, 0.5, 1.0)
    ...
    fieldOutputController.addExpressionFieldOutput(
        associatedSet=theModel.nodeSets["bottom_nodes"],
        theExpression=lambda: np.abs(dirichletBottom.perNodePenaltyForce[:, 1]),
        name="bottom_normal_force",
    )
    fieldOutputController.addExpressionFieldOutput(
        associatedSet=theModel.nodeSets["bottom_nodes"],
        theExpression=lambda: np.abs(dirichletBottom.perNodePenaltyForce[:, 0]),
        name="bottom_tangential_force",
    )
    ...
    ensightOutput = EnsightOutputManager(
        out_folder, theModel, fieldOutputController, theJournal, None, intermediateSaveInterval=save_interval
    )
    ...
    ensightOutput.updateDefinition(
        fieldOutput=fieldOutputController.fieldOutputs["bottom_normal_force"], create="perNode"
    )
    ensightOutput.updateDefinition(
        fieldOutput=fieldOutputController.fieldOutputs["bottom_tangential_force"], create="perNode"
    )
