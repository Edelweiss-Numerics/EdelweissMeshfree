...
def validate_results():
    ...
    for t_val in time_values:
        ...
        print(f"t={t_val:.4f} ({status}): expected rigid Y disp = {expected_disp:.4f}, actual rigid Y disp = {np.min(actual_disp):.4f}, block max disp = {max_block_disp:.4f}")
        
        if max_error > 1e-3:
            errors.append(f"At t={t_val}, rigid displacement mismatched: expected {expected_disp}, got max diff {max_error}")
