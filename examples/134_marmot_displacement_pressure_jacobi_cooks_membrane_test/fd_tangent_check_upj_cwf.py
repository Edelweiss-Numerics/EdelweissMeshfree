"""FD verification of the CWF-correction boundary term of the u-p-J particle:
d(fExt_cwf)/ddQ vs the analytic dFExt_ddQ, chained through computePhysicsKernels
(which sets the material-point state the boundary term reads)."""
...
def main():
    ...
    for p in (parts[0], parts[2]):  # boundary particles (left edge region)
        ...
        def cwf(dQ, faceID=4):
            ...
            p.computeDistributedLoad("CWFCORRECTION", faceID, np.array([0.0]), Pc, Kc, 1.0, 1.0)
            return Pc, Kc
